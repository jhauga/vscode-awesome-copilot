    ...
    constructor(
        ...
        if (itemType === 'search') {
            ...
            if (copilotItem.category === CopilotCategory.Skills && copilotItem.file.type === 'dir') {
                ...
                // Update description with update indicator if applicable
                const sizeDesc = `${(copilotItem.file.size / 1024).toFixed(1)}KB`;
                this.description = hasUpdate ? `🔄 ${sizeDesc} (Update Available)` : sizeDesc;
                
                this.tooltip = new vscode.MarkdownString(
                    `**${copilotItem.name}**\n\nSize: ${(copilotItem.file.size / 1024).toFixed(1)}KB\nRepo: ${copilotItem.repo ? copilotItem.repo.owner + '/' + copilotItem.repo.repo : ''}${isDownloaded ? '\n\n✅ Downloaded' : ''}${hasUpdate ? '\n\n⚠️ **Update Available** - A newer version is available on the remote repository. Download again to get the latest version.' : ''}\n\nClick to preview content`
                );
